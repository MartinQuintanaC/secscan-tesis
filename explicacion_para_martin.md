...
## 8️⃣ Rediseño de Escaneo Pasivo a Demonio en Segundo Plano (Aceleración Activa)

En la última actualización de arquitectura, cambiamos radicalmente el enfoque del escaneo pasivo para convertirlo en un proceso de fondo continuo y automatizado que asiste al escaneo activo:

*   **Daemon de Descubrimiento de Fondo (`PassiveScanDaemon`)**:
    *   **Inicialización automática**: El backend inicializa un hilo persistente (`Thread` en modo daemon) de manera autónoma al arrancar la aplicación (vía evento `startup` de FastAPI). No requiere intervención del usuario.
    *   **Funcionamiento continuo**: Cada 30 segundos, el hilo despierta, comprueba la subred local actual mediante `get_local_cidr()` (si cambia, borra la caché), lee la tabla de caché ARP del sistema operativo (`arp -a`) y realiza un refresco silencioso en segundo plano.
    *   **Caché global persistente**: Mantiene en memoria una colección actualizada de todos los hosts activos conocidos de la red local.
*   **Aceleración del Escaneo Activo**:
    *   Cuando el usuario ejecuta un escaneo activo, en lugar de ordenar a Nmap que barra la subred completa (256 direcciones IP en un rango `/24`), la capa de servicio extrae las IPs confirmadas como encendidas por el demonio pasivo en segundo plano.
    *   Nmap se ejecuta dirigiendo sus sondas **únicamente hacia esas IPs activas**. Esto disminuye el tiempo de descubrimiento activo de ~20-30 segundos a solo **2-3 segundos**, evitando esperas y timeouts en IPs muertas.
*   **Resultados Pasivos Instantáneos**:
    *   Al solicitar un escaneo pasivo, el backend no realiza esperas; simplemente retorna de forma instantánea (0 segundos) el estado de la red almacenado en la caché del demonio de fondo.
